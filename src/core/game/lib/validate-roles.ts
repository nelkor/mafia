import { checkVictory } from '../../features'
...
export const validateRoles = (roles: GameRole[]) => {
  ...
  const catcherWithoutShooters = shooters
    ? ''
    : bodyguard
      ? 'bodyguard'
      : ricochet
  ...
  if (catcherWithoutShooters) {
    return `Forbidden to have a ${catcherWithoutShooters} without any shooters`
  }
  ...
  if (don && !sheriff) {
    return 'Forbidden to have a don without a sheriff'
  }
